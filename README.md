...
**[🔗 배포된 웹](https://heebon00.github.io/yt_music/)** · **[🎨 Figma 프로토타입](https://www.figma.com/proto/0OKoegX3OBJMciNeO2rdgS/?node-id=211-1559&starting-point-node-id=211-1559)** · **[📑 디자인 덱](https://www.figma.com/design/0OKoegX3OBJMciNeO2rdgS/)**
...
- **아트워크** 실제 앨범 자켓을 표시합니다. 이미지를 저장소에 복사하지 않고 외부 CDN 주소를 참조하며,
  로딩 전·실패 시에는 곡별 그라디언트가 대신 보입니다 (`Cover.jsx`)
- **브랜드 로고 & 스플래시** 음악 플레이어의 보편적 심볼(원형 디스크 + 재생 삼각형)과 브랜드 악센트 컬러(`#CF032A`)를 조합한 미니멀 로고를 적용했습니다. 모바일 첫 진입 시 은은한 펄스 효과의 **스플래시 로딩 화면 (`SplashScreen.jsx`)**이 부드럽게 나타나며 앱의 첫인상 완성도를 높입니다.
...
**구현 완성도를 높인 결정.**
검색창은 실시간 라이브 검색(곡·아티스트·재생목록 필터링 및 즉시 재생)을 지원하여 탐색 흐름이 끊기지 않게 구현했습니다. 실제 오디오 스트리밍 재생 백엔드 연동을 제외한 모든 UI 인터랙션과 상태를 완결성 있게 동작하도록 맞췄습니다.
...
- **앨범 자켓 이미지의 저작권은 각 아티스트와 음반사에 있습니다.** 화면 구현을 보여주기 위한 목적으로만
  표시하며, 저장소에 이미지를 포함하지 않고 외부 주소를 참조합니다. 권리자 요청 시 즉시 내립니다
