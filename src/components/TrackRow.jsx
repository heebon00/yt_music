...
export default function TrackRow({ item, onPlay, active = false }) {
  return (
    <div className="group flex items-center gap-4 px-5 py-2.5 transition-colors hover:bg-surface">
      <button type="button" onClick={() => onPlay?.(item)} className="flex min-w-0 flex-1 items-center gap-4 text-left">
        <div className="size-11 shrink-0 rounded-md" style={artStyle(item.art)} />
